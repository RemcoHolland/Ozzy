...
bool Square::isAttacked(int color, Board board, uint64_t square) {
    ...
    return (pawnAttacks & board.getPiece(BLACK_PAWN - king_color)) ||
           (knightAttacks & board.getPiece(BLACK_KNIGHT - king_color)) ||
           (bishopAttacks & board.getPiece(BLACK_BISHOP - king_color)) ||
           (rookAttacks & board.getPiece(BLACK_ROOK - king_color)) ||
           (queenAttacks & board.getPiece(BLACK_QUEEN - king_color)) ||
           (kingAttacks & board.getPiece(BLACK_KING - king_color));
}
